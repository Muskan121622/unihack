...
Outputs:
    final_delivery_1000_rows.csv      — 252-column delivery CSV
    evidence_debug_1000_rows.json     — per-product pipeline diagnostics
...
OUT_CSV      = os.path.join(_ROOT, "final_delivery_1000_rows.csv")
OUT_DEBUG    = os.path.join(_ROOT, "evidence_debug_1000_rows.json")
MAX_ROWS     = 1000
...
def check_internet():
    try:
        ...
        print("[!!!] Aborting to prevent corrupted CSVs. Reconnect and restart. [!!!]")
        sys.exit(1)
...
def main():
    check_internet()
    banner("v2 Pipeline — 1000-Row Test")
    ...
    for idx, input_row in enumerate(rows_to_run):
        ...
        if chunks and os.getenv("VOYAGE_API_KEY"):
            ...
            # THE EYE: Voyage Document Embed Validation
            if embed_stats.get("chunks_failed", 0) > 0:
                print(f"\n[!] Voyage API exhausted/failed while embedding {mpn}.")
                print(f"[!] Skipping saving CSV so it will automatically retry next time!")
                continue
        ...
        if os.getenv("VOYAGE_API_KEY"):
            ...
            # THE EYE: Voyage Query Embed Validation
            if ret_stats.get("query_embed_failures", 0) > 0:
                print(f"\n[!] Voyage API exhausted/failed while querying {mpn}.")
                print(f"[!] Skipping saving CSV so it will automatically retry next time!")
                continue
        ...
        llm_out, llm_stats = {}, {"model_used":None,"prompt_chars":0,"success":False,"error":"GEMINI_API_KEY not set"}
        if os.getenv("GEMINI_API_KEY") or os.getenv("GROQ_API_KEY"):
            print(f"\n[S7] LLM extraction...")
            llm_out, llm_stats = s7.extract_schema(mpn, brand, desc, ev, retrieved)
            
            # THE EYE: Instantly catch LLM failures and prevent saving empty/broken CSVs!
            if not llm_stats.get("success"):
                print(f"\n[!] LLM completely failed for {mpn} (Rate Limit / Error).")
                print(f"[!] Skipping saving CSV so the pipeline will automatically retry this row next time!")
                continue
                
            json.dump(llm_out, open(os.path.join(art_dir,"v2_llm_output.json"),"w",encoding="utf-8"), indent=2, ensure_ascii=False)
        else:
            print(f"\n[S7] API_KEY not set - skip LLM")
    ...
    try:
        ...
        print(f"[!] Could not convert to Excel automatically: {e}")

    print("\nNext: inspect the final delivery files!")
